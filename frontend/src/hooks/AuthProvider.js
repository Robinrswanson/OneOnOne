...
const AuthProvider = ({ children }) => {
  ...
  const loginAction = async (data) => {
    try {
      const response = await axios.post("http://ec2-3-95-237-137.compute-1.amazonaws.com/accounts/login/", data);
  ...
  const registerAction = async (data) => {
    try {
      await axios.post("http://ec2-3-95-237-137.compute-1.amazonaws.com/accounts/register/", data);
